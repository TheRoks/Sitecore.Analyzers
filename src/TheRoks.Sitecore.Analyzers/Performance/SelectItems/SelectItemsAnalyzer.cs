...
namespace TheRoks.Sitecore.Analyzers.Performance.SelectItems
{
	...
	public class SelectItemsAnalyzer : DiagnosticAnalyzer
	{
		...
		private static readonly DiagnosticDescriptor Rule = new DiagnosticDescriptor(Helper.ToDiagnosticId(DiagnosticIds.SelectItems),
																					 Title,
																					 MessageFormat,
		...
		private void AnalyzeSyntaxNode(SyntaxNodeAnalysisContext context)
		{
			...
			if (memberAccessExpr?.Name == null || memberAccessExpr.Name.ToString() != "SelectItems")
			{
				return;
			}
			...
			if (!memberSymbol.ToString().StartsWith("Sitecore.Data.Database.SelectItems"))
			{
				return;
			}
